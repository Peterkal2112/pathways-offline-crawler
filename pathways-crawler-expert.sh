...
# 3. Preparation
printf "\n\033[0;32m[1/4] Preparing directory: %s\033[0m\n" "$TARGET_DIR"
mkdir -p "$TARGET_DIR"
cd "$TARGET_DIR" || exit

# 4. Bootstrap Discovery Files
printf "\033[0;32m[2/4] Downloading entry files into proper subdirectories...\033[0m\n"
BASE="https://www.shoutoutuk.org/gamepw/"

# Create the folder structure mirroring Articulate's engine
...
# Clean up any misplaced files from root
rm -f data.js paths.js

# 5. The Crawling Bit (Strict Path Preservation)
printf "\033[0;32m[3/4] Starting Recursive Asset Discovery...\033[0m\n"
...
def deep_scan():
    ...
    # Identify all crawlable files (html, js, css, etc.)
    ...
    # Extract links from those files
...
round_num = 1
while deep_scan():
    print(f'[*] Round {round_num} complete. Scanning newly found files...')
    round_num += 1

print(f'[*] Final Sync Complete: {total_new} files added.')
\" "

# 6. Final Step: Webserver
FILE_COUNT=$(find "$TARGET_DIR" -type f | wc -l)
printf "\n\033[0;32m[4/4] Integrity Check: %s files found.\033[0m\n" "$FILE_COUNT"
